...
BOT_TOKEN = "8697660848:AAGV66xqwnsN89UJdvZ3qOZiHLDA3H7TrxM"
ADMIN_IDS = [6528792525]
...
async def get_8_plans_keyboard():
    ...
    for pid, name, price, _ in plans:
        builder.button(
            text=f"🔥 {name} (Rs.{int(price)})", callback_data=f"buy_plan:{pid}"
        )
    builder.button(text="🏠 Home", callback_data="btn_home")
...
async def nav_home(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Deletes all tracked messages (both welcome message and plan message) when Home is clicked
...
async def process_payment_proof(message: types.Message, state: FSMContext):
    ...
    for admin_id in ADMIN_IDS:
        try:
            builder = InlineKeyboardBuilder()
            builder.button(
                text="Approve", callback_data=f"adm_pay:{pid}:approved"
            )
            builder.button(
                text="Reject", callback_data=f"adm_pay:{pid}:rejected"
            )
            builder.adjust(2)
